...
namespace _8.EarlyWarningResult
{
    public partial class EarlyWarning : Form
    {
        ...
        public void LoadValue(string strTime)
        {
            ...
            int valueCount = workingfaceEntWarning.Count;
            for (int i = 0; i < valueCount; i++)
            {
                addRow2Fp(FROZEN_ROW_COUNT + i, workingfaceEntWarning[i]);
            }

            valueCount = workingfaceEntNormal.Count;
            int iTmpRowCount = FROZEN_ROW_COUNT + workingfaceEntWarning.Count;

            for (int i = 0; i < valueCount; i++)
            {
                addRow2Fp(iTmpRowCount + i, workingfaceEntNormal[i]);
            }
        ...
        private void _fpPreWaringLastedValue_CellClick(object sender, FarPoint.Win.Spread.CellClickEventArgs e)
        {
            ...
            if (e.Column == 14)
            {
                ...
                warningIdList = PreWarningLastedResultQueryBLL.GetWarningIdListWithWorkingfaceName(workface);
                ...
                EarlyWarningDetails ewDetailDlg = new EarlyWarningDetails(_outInfo, false);
                ewDetailDlg.ShowDialog();
