...
{
    public partial class EarlyWarningDetails : Form
    {
        ...
        public EarlyWarningDetails(string sOutTunnelId,
        ...
        public EarlyWarningDetails(EarlyWarningResultEntity inInfo, bool isHistory)
        {
            InitializeComponent();
                        ...
                        string sOutWarningItem)
        {
            ...
            List<WarningResultDetailEntity> warningResultDetails = getHistoryWarningResultDetails(sOutWorkface,
        ...
        private void LoadWarningResultDetail(List<string> warningIdList)
        {
            ...
            List<WarningResultDetailEntity> warningResultDetails = getHistoryWarningResultDetails(warningIds);
            // 向farpoint spread中添加数据。
            for (int j = 0; j < warningResultDetails.Count; j++)
            {
                addOneRowToFpSpread(_iRowCount++, warningResultDetails[j], this._fpPreWarningResultDetials);
        ...
        private void doActions()
        {
            ...
            List<NameValuePair> nameValuePairList = new List<NameValuePair>();
            nameValuePairList.Add(new NameValuePair(OPERATION, OPERATION_ACTION));
            string jsonStr = Newtonsoft.Json.JsonConvert.SerializeObject(container);
            nameValuePairList.Add(new NameValuePair("jsonmsg", jsonStr));
            UrlEncodedFormEntity formEntity = new UrlEncodedFormEntity(nameValuePairList, Encoding.UTF8);
            postMethod.Entity = formEntity;
        ...
        private void doComments()
        {
            ...
            HttpPost postMethod = new HttpPost(new Uri("http://" + serverIp + ":" + restPort + "/rest/tunnels/" + _inInfo.TunnelID + "/warninglift"));
            List<NameValuePair> nameValuePairList = new List<NameValuePair>();
            nameValuePairList.Add(new NameValuePair(OPERATION, OPERATION_COMMENTS));
            string jsonStr = Newtonsoft.Json.JsonConvert.SerializeObject(container);
            nameValuePairList.Add(new NameValuePair("jsonmsg", jsonStr));
            UrlEncodedFormEntity formEntity = new UrlEncodedFormEntity(nameValuePairList, Encoding.UTF8);
            postMethod.Entity = formEntity;
            ...
            if (responseCode < 400)
            {
                ...
                byte[] buffer = response.Entity.Content;

                // From byte array to string
                string s = Encoding.UTF8.GetString(buffer, 0, buffer.Length);

                MessageBox.Show("提交成功！" + s);

                lblStatus.Text = "评价成功，预警待解除";
                this.btnDoIt.Enabled = false;
            }
            else
            {
        ...
        private void doWarningLift()
        {
            ...
            HttpPost postMethod = new HttpPost(new Uri("http://" + serverIp + ":" + restPort + "/rest/tunnels/" + _inInfo.TunnelID + "/warninglift"));
            List<NameValuePair> nameValuePairList = new List<NameValuePair>();
            nameValuePairList.Add(new NameValuePair(OPERATION, OPERATION_WARNING_LIFT));
            string jsonStr = Newtonsoft.Json.JsonConvert.SerializeObject(container);
            nameValuePairList.Add(new NameValuePair("jsonmsg", jsonStr));
            UrlEncodedFormEntity formEntity = new UrlEncodedFormEntity(nameValuePairList, Encoding.UTF8);
            postMethod.Entity = formEntity;
        ...
        private void btnLiftWarning_Click(object sender, EventArgs e)
        {
            ...
            {
                if (this.sHandleStatus == "0")
                    doActions(); //提交解除措施
                else if (this.sHandleStatus == "1")
                    doComments(); // 提交措施评价
                else if (this.sHandleStatus == "2")
                ...
                //    ent.FileName = ImgList[i];
                //    ent.Remarks = "";
                //    ent.WarningId = warningId;
                //    ent.Img = GetBytesByImagePath(ImgList[i]);
                //    if (!WarningImgBLL.IsRepeatWithWarningIdAndFileName(warningId, ent.FileName))
                //    {
