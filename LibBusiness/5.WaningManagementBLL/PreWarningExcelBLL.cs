...
namespace LibBusiness
{
    ...
    {
        private static string _excelFilePath = System.Windows.Forms.Application.StartupPath + "\\" + "PreWarningRules.xlsx";

        public static bool ImportExcelRules2Db(string excelPath)
        {
            string[] sheetNames = PreWarningExcelBLL.GetExcelSheetNames(excelPath);
            if (sheetNames != null)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    DataSet dsSheet = LibExcelHelper.importExcelSheetToDataSet(excelPath, sheetNames[i]);
                    if (dsSheet == null)
                    {
                        return false;
                    }
                    for (int j = 0; j < dsSheet.Tables[0].Rows.Count; j++)
                    {
                        ...
                        if (!PreWarningRulesBLL.InsertPreWarningRulesInfo(ent))
                        {
                            if (DialogResult.Yes == MessageBox.Show("写入Excel预警规则至数据库失败!\r\n是否继续？", "Sheet：" + sheetNames[i] + " 行:" + (j + 1).ToString(), MessageBoxButtons.YesNo))
                            {
                                continue;
                            }
                            else
        ...
        public static string[] GetExcelSheetNames(string excelPath)
        {
            ...
            try
            {
                ...
                objExcelWorkBook = objExcelWorkBooks.Open(excelPath, 0, true, 5, "", "", true,
                ...
                for (int i = 1; i <= n; i++)
                {
                    Excel.Worksheet objExcelWorkSheet = (Worksheet)objExcelWorkBook.Worksheets[i]; //(Worksheet)objExcelWorkBook.Sheets[i];
        ...
        public static PreWarningRules ConvertExcelDataRow2PreWarningEntity(DataRow dr, out string errorMsg)
        {
            ...
            try
            {
                ...
                {
                    errorMsg = "预警规则Excel表中规则编码为空，请检查并删除Excel表中的空行等无效数据！";//删除方法：光标移至数据的最后一行，然后按Ctrl+Shift+End选择空行数据，右键鼠标-删除
                    return null;
                }
                //规则编码
                ...
                {
                    errorMsg = "预警规则Excel表中指定的修改日期格式不正确，将使用当前日期！【规则编码:" + dr[0].ToString() + "】";
        ...
        public static bool updateInfoToExcelSheet(PreWarningRules ent, int selectedRowID, string sheetName)
        {
            ...
            try
            {
                objExcelApp = new Excel.ApplicationClass();
                objExcelWorkBooks = objExcelApp.Workbooks;
                objExcelWorkBook = objExcelWorkBooks.Open(_excelFilePath, 0, false, 5, "", "", true,
                    Microsoft.Office.Interop.Excel.XlPlatform.xlWindows, "", true, false, 0, true, false, false);
                objExcelWorkSheet = (Worksheet)objExcelWorkBook.Worksheets[sheetName];
                objExcelWorkSheet.Select(Type.Missing);

                Worksheet objExcelWorkSheetTemp = (Worksheet)objExcelApp.ActiveSheet;
                objExcelWorkSheetTemp.Cells[selectedRowID + 1, 1] = ent.RuleCode;
                objExcelWorkSheetTemp.Cells[selectedRowID + 1, 2] = ent.RuleType;
                objExcelWorkSheetTemp.Cells[selectedRowID + 1, 3] = ent.WarningType;
                objExcelWorkSheetTemp.Cells[selectedRowID + 1, 4] = ent.WarningLevel;
                objExcelWorkSheetTemp.Cells[selectedRowID + 1, 5] = ent.SuitableLocation;
                objExcelWorkSheetTemp.Cells[selectedRowID + 1, 6] = ent.RuleDescription;
                objExcelWorkSheetTemp.Cells[selectedRowID + 1, 7] = ent.IndicatorType;
                objExcelWorkSheetTemp.Cells[selectedRowID + 1, 8] = ent.Operator;
                objExcelWorkSheetTemp.Cells[selectedRowID + 1, 9] = ent.ModifyDate.ToString();
                objExcelWorkSheetTemp.Cells[selectedRowID + 1, 10] = ent.Remarks;//MARK FIELD
