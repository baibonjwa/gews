...
namespace LibBusiness
{
    public class GasContentBLL
    {
        ...
        public static bool insertGasContentInfo(GasContent 
            gasContentEntity)
        {
            StringBuilder sqlStr = new StringBuilder();
            sqlStr.Append("INSERT INTO " + 
                GasContentDbConstNames.TABLE_NAME);
            ...
            sqlStr.Append(", '" + 
                gasContentEntity.MeasureDateTime.ToString("yyyy-MM-dd hh:mm:ss") 
                + "'");
        ...
        /// <returns></returns>
        public static DataSet selectGasContentInfoForPage(int iStartIndex, 
            int iEndIndex)
        {
            StringBuilder sqlStr = new StringBuilder();
            sqlStr.Append("SELECT * FROM (");
            sqlStr.Append(" SELECT ROW_NUMBER() OVER(ORDER BY " + 
                GasContentDbConstNames.ID + ") AS rowid, *");
            sqlStr.Append(" FROM " + GasContentDbConstNames.TABLE_NAME + 
                ") AS TB");
        ...
        public static DataSet selectGasContentInfoByPK(int iPrimaryKey)
        {
            ...
            sqlStr.Append("SELECT * FROM " + 
                GasContentDbConstNames.TABLE_NAME);
            sqlStr.Append(" WHERE " + GasContentDbConstNames.ID + " = " + 
                iPrimaryKey);
        ...
        /// <param name="gasContentEntity">【瓦斯含量点】实体</param>
        /// <returns>成功与否：true，false</returns>
        public static bool updateGasContentInfo(GasContent 
            gasContentEntity)
        {
            StringBuilder sqlStr = new StringBuilder();
            ...
            sqlStr.Append(" SET");
            sqlStr.Append("  " + GasContentDbConstNames.X + " = '" + 
                gasContentEntity.CoordinateX + "'");
            sqlStr.Append(", " + GasContentDbConstNames.Y + " = '" + 
                gasContentEntity.CoordinateY + "'");
            sqlStr.Append(", " + GasContentDbConstNames.Z + " = '" + 
                gasContentEntity.CoordinateZ + "'");
            sqlStr.Append(", " + GasContentDbConstNames.DEPTH + " = '" + 
                gasContentEntity.Depth + "'");
            sqlStr.Append(", " + GasContentDbConstNames.GAS_CONTENT_VALUE + 
                " = '" + gasContentEntity.GasContentValue + "'");
            sqlStr.Append(", " + GasContentDbConstNames.MEASURE_DATE_TIME + 
                " = '" + gasContentEntity.MeasureDateTime + "'");
            sqlStr.Append(", " + GasContentDbConstNames.TUNNEL_ID + " = '" 
                + gasContentEntity.Tunnel + "'");
            sqlStr.Append(", " + GasContentDbConstNames.COAL_SEAMS_ID + 
                " = '" + gasContentEntity.CoalSeams + "'");
            sqlStr.Append(" WHERE " + GasContentDbConstNames.ID + " = '" + 
                gasContentEntity.PrimaryKey + "'");
        ...
        public static void gasValuePointToTxt(string strExportPointPath, 
            int coalSeamID)
        {
            SaveDBXyz2Txt.WriteXYZ(strExportPointPath, 
                GasContentDbConstNames.TABLE_NAME, GasContentDbConstNames.X, 
                GasContentDbConstNames.Y, 
                GasContentDbConstNames.GAS_CONTENT_VALUE, coalSeamID);
        }
    }
}
