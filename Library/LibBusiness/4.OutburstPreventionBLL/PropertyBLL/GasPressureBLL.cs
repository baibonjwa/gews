...
namespace LibBusiness
{
    public class GasPressureBLL
    {
        ...
        public static bool insertGasPressureInfo(GasPressure 
            gasPressureEntity)
        {
            StringBuilder sqlStr = new StringBuilder();
            sqlStr.Append("INSERT INTO " + 
                GasPressureDbConstNames.TABLE_NAME);
            ...
            sqlStr.Append(", " + 
                GasPressureDbConstNames.GAS_PRESSURE_VALUE);
            sqlStr.Append(", " + 
                GasPressureDbConstNames.MEASURE_DATE_TIME);
        ...
        /// <returns></returns>
        public static DataSet selectGasPressureInfoForPage(int iStartIndex, 
            int iEndIndex)
        {
            StringBuilder sqlStr = new StringBuilder();
            sqlStr.Append("SELECT * FROM (");
            sqlStr.Append(" SELECT ROW_NUMBER() OVER(ORDER BY " + 
                GasPressureDbConstNames.ID + ") AS rowid, *");
            sqlStr.Append(" FROM " + GasPressureDbConstNames.TABLE_NAME + 
                ") AS TB");
        ...
        public static DataSet selectGasPressureInfoByPK(int iPrimaryKey)
        {
            ...
            sqlStr.Append("SELECT * FROM " + 
                GasPressureDbConstNames.TABLE_NAME);
            sqlStr.Append(" WHERE " + GasPressureDbConstNames.ID + " = " + 
                iPrimaryKey);
        ...
        /// <returns>成功与否：true，false</returns>
        public static bool updateGasPressureInfo(GasPressure 
            gasPressureEntity)
        {
            StringBuilder sqlStr = new StringBuilder();
            ...
            sqlStr.Append(" SET");
            sqlStr.Append("  " + GasPressureDbConstNames.X + " = '" + 
                gasPressureEntity.CoordinateX + "'");
            sqlStr.Append(", " + GasPressureDbConstNames.Y + " = '" + 
                gasPressureEntity.CoordinateY + "'");
            sqlStr.Append(", " + GasPressureDbConstNames.Z + " = '" + 
                gasPressureEntity.CoordinateZ + "'");
            sqlStr.Append(", " + GasPressureDbConstNames.DEPTH + " = '" + 
                gasPressureEntity.Depth + "'");
            sqlStr.Append(", " + GasPressureDbConstNames.GAS_PRESSURE_VALUE 
                + " = '" + gasPressureEntity.GasPressureValue + "'");
            sqlStr.Append(", " + GasPressureDbConstNames.MEASURE_DATE_TIME 
                + " = '" + gasPressureEntity.MeasureDateTime + "'");
            sqlStr.Append(", " + GasPressureDbConstNames.TUNNEL_ID + " = '" 
                + gasPressureEntity.Tunnel + "'");
            sqlStr.Append(", " + GasPressureDbConstNames.COAL_SEAMS_ID + 
                " = '" + gasPressureEntity.CoalSeams + "'");
            sqlStr.Append(" WHERE " + GasPressureDbConstNames.ID + " = " + 
                gasPressureEntity.PrimaryKey);
        ...
        /// 将数据库中瓦斯压力点数据存至文本文件
        /// </summary>
        public static void gasPressurePointToTxt(string strExportPointPath, 
            int coalSeamID)
        {
            SaveDBXyz2Txt.WriteXYZ(strExportPointPath, 
                GasPressureDbConstNames.TABLE_NAME, GasPressureDbConstNames.X, 
                GasPressureDbConstNames.Y, 
                GasPressureDbConstNames.GAS_PRESSURE_VALUE, coalSeamID);
        }
    }
}
