...
namespace LibBusiness
{
    public class LoginFormBLL
    {
        ...
        public static UserLogin GetUserLoginInformationByLoginname(string 
            loginname)
        {
            UserLogin name = null;
            StringBuilder strSql = new StringBuilder();
            ...
            strSql.Append(" =  ");
            strSql.Append("'" + 
                LibEncryptDecrypt.DWEncryptDecryptClass.EncryptString(loginname) 
                + "'");
            ManageDataBase database = new 
                ManageDataBase(DATABASE_TYPE.WarningManagementDB);
            DataTable dt = database.ReturnDS(strSql.ToString()).Tables[0];
        ...
        /// <returns></returns>
        public static UserLogin LoginSuccess(string loginname, string 
            passward)
        {
            UserLogin name = null;
            StringBuilder strSql = new StringBuilder();
            ...
            strSql.Append(" = ");
            strSql.Append("'" + 
                LibEncryptDecrypt.DWEncryptDecryptClass.EncryptString(passward) 
                + "'");
            ManageDataBase database = new 
                ManageDataBase(DATABASE_TYPE.WarningManagementDB);
            DataTable dt = database.ReturnDS(strSql.ToString()).Tables[0];
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                name.LoginName = 
                    LibEncryptDecrypt.DWEncryptDecryptClass.DecryptString(dt.Rows[0][1].ToString());
                name.PassWord = 
                    LibEncryptDecrypt.DWEncryptDecryptClass.DecryptString(dt.Rows[0][2].ToString());
                name.Permission = 
                    LibEncryptDecrypt.DWEncryptDecryptClass.DecryptString(dt.Rows[0][3].ToString());
                name.GroupName = 
                    LibEncryptDecrypt.DWEncryptDecryptClass.DecryptString(dt.Rows[0][4].ToString());
                name.IsSavePassWord = Convert.ToInt32(dt.Rows[0][5]);
                name.IsLogined = Convert.ToInt32(dt.Rows[0][6].ToString());
                name.Remarks = 
                    LibEncryptDecrypt.DWEncryptDecryptClass.DecryptString(dt.Rows[0][7].ToString());
            }
        ...
        public static UserLogin 
            GetUserLoginInformationByIDAndLoginName(string id, string 
            loginname)
        {
            UserLogin name = null;
            StringBuilder strSql = new StringBuilder();
            ...
            if (dt != null)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    ...
                    name.GroupName = 
                        LibEncryptDecrypt.DWEncryptDecryptClass.DecryptString(dt.Rows[i][4].ToString());
                    name.IsSavePassWord = 
                        Convert.ToInt32(dt.Rows[i][5].ToString());
                    name.IsLogined = 
                        Convert.ToInt32(dt.Rows[i][6].ToString());
                    name.Remarks = 
                        LibEncryptDecrypt.DWEncryptDecryptClass.DecryptString(dt.Rows[i][7].ToString());
                }
            }
        ...
        public static bool DeleteUserLoginInformationByLoginName(string 
            name)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        public static bool UpdateUserLoginInfomation(UserLogin ent, string 
            oldName)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            strSql.Append("'" + 
                LibEncryptDecrypt.DWEncryptDecryptClass.EncryptString(ent.GroupName) 
                + "',");
            ...
            strSql.Append("'" + 
                LibEncryptDecrypt.DWEncryptDecryptClass.EncryptString(ent.Remarks) 
                + "'");
            strSql.Append(" WHERE ");
            strSql.Append(LoginFormDbConstNames.USER_LOGIN_NAME + " = ");
            strSql.Append("'" + 
