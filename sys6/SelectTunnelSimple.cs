...
namespace UnderTerminal
{
    public partial class SelectTunnelSimple : UserControl
    {
        ...
        {
            if (File.Exists(@"RecentTunnels.xml"))
            {
                var doc = new XmlDocument();
                doc.Load(@"RecentTunnels.xml");

                var nodes = doc.DocumentElement.SelectNodes("/Tunnels/Tunnel");

                //List<TunnelSimple> tunnels = new List<TunnelSimple>();

                var firstTS = new TunnelSimple(-1, "已选择的巷道");
                ...
                foreach (XmlNode node in nodes)
                {
                    ...
                    var tunnel = new TunnelSimple(int.Parse(id), name);
        ...
        private void btnChooseTunnel_Click(object sender, EventArgs e)
        {
            ...
            if (DialogResult.OK == dlg.ShowDialog())
            {
                ...
                foreach (TunnelSimple tunnel in cbxTunnel.Items)
                {
                    index++;
                ...
                var ts = new TunnelSimple(dlg.tunnelId, dlg.tunnelName);
                ...
                var writer = XmlWriter.Create("RecentTunnels.xml", settings);
                writer.WriteStartDocument();
                writer.WriteComment("This file is generated by the program.");

                writer.WriteStartElement("Tunnels");

                for (var i = 1; i < cbxTunnel.Items.Count; i++)
                {
                    var tunnel = cbxTunnel.Items[i] as TunnelSimple;
        ...
        {
            var ts = cbxTunnel.SelectedItem as TunnelSimple;
            _iTunnelId = ts.Id;
            tunnelName = ts.Name;
        ...
        private void cbxTunnel_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Delete)
            {
                try
                {
                    var currentItem = cbxTunnel.SelectedIndex;
                    if (currentItem > 0)
                    {
