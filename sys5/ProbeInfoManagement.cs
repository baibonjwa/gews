    ...
    public partial class ProbeInfoManagement : BaseForm
    {
            ...
            if (iSelCnt > 0)
            {
                ...
                for (int i = 0; i < iSelCnt; i++)
                {
                    ...
                    fpProbeInfo.Sheets[0].Cells[_iRowDetailStartIndex + i, ++index].Text =
                        probes[i].ProbeType.ProbeTypeName;
                    ...
                    // 探头描述
                    fpProbeInfo.Sheets[0].Cells[_iRowDetailStartIndex + i, ++index].Text = probes[i].ProbeDescription;
                    ...
                    {
                        fpProbeInfo.Sheets[0].Cells[_iRowDetailStartIndex + i, ++index].Text =
                            probes[i].Tunnel.WorkingFace.MiningArea.Horizontal.Mine.MineName;
                        // 水平
                        fpProbeInfo.Sheets[0].Cells[_iRowDetailStartIndex + i, ++index].Text =
                            probes[i].Tunnel.WorkingFace.MiningArea.Horizontal.HorizontalName;
                        // 采区
                        fpProbeInfo.Sheets[0].Cells[_iRowDetailStartIndex + i, ++index].Text =
                            probes[i].Tunnel.WorkingFace.MiningArea.MiningAreaName;
                        // 工作面
                        fpProbeInfo.Sheets[0].Cells[_iRowDetailStartIndex + i, ++index].Text =
                            probes[i].Tunnel.WorkingFace.WorkingFaceName;
                        // 巷道名称
                        fpProbeInfo.Sheets[0].Cells[_iRowDetailStartIndex + i, ++index].Text =
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (DialogResult.OK == probeInfoEnteringForm.ShowDialog())
            {
                ...
                // 设置farpoint焦点（必须实装）
                fpProbeInfo.Sheets[0].SetActiveCell(fpProbeInfo.Sheets[0].Rows.Count, 0);
            }
        }
