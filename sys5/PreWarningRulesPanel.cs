...
namespace _5.WarningManagement
{
    public partial class PreWarningRulesPanel : Form
    {
        ...
        private const string CATEGORY_BASICINFO_TITLE = "1.基本信息";
        private const string CATEGORY_RULE_PARAMS_TITLE = "2.规则参数";
        private const string CATEGORY_OPERATOR_TYPE_TITLE = "3.计算方法";
        private const string CATEGORY_INDICATORR_TYPE_TITLE = "4.指标类型";
        private const string CATEGORY_OTHERS_TITLE = "5.其他";
        private const string CATEGORY_REMARKS_TITLE = "6.备注";
        private const string CATEGORY_MULTIRULES_BINDING_SINGLERULE_TITLE = "6.组合规则"; //MAKR FIELD
        //数据行数
        ...
        private PreWarningRules _srcEnt;
        private int DATA_COLUMN_CNT = 9; //可用数据的列数 MARK FIELD
        //修改日期
        private readonly XProp _dateProp = new XProp();
        //需要过滤的列索引
        private readonly int[] _filterColunmIdxs;
        //指标类型
        private readonly XProp _indicatorType = new XProp();
        //该规则是否是组合规则
        private readonly XProp _IsMultiRules = new XProp();
        //组合规则中绑定的单一规则
        private readonly XProp _multiRulesBindingSingleRule = new XProp();
        //比较符
        private readonly XProp _operator = new XProp(); //MARK FIELD
        private readonly XProp _paramCnt = new XProp();
        private readonly List<XProp> _paramProp = new List<XProp>();
        private readonly XProps _props = new XProps();
        //备注
        private readonly XProp _remarks = new XProp();
        //规则ID
        private readonly XProp _ruleCodeId = new XProp();
        //规则编码
        private readonly XProp _ruleCodeProp = new XProp();
        //规则类别
        private readonly XProp _ruleType = new XProp();
        //适用位置
        private readonly XProp _suitableLocation = new XProp();
        //预警级别
        private readonly XProp _warningLevel = new XProp();
        //预警类型
        private readonly XProp _warningType = new XProp();
        ...
        {
            //适用位置
            var locations = Const_WM.GetSuitableLocations();
            cmbFilterLocation.Items.AddRange(locations);
            //初始时不设置过滤选项，否则会自动调用SelectedIndexChange函数，导致farpoint重复加载
            //this.cmbFilterLocation.SelectedIndex = locations.Length - 1;

            //预警类型
            var warningTypes = Const_WM.GetWarningTypeConstStrings();
            cmbFilterWarningType.Items.AddRange(warningTypes);
            cmbFilterWarningType.Items.Add(Const_WM.WARNING_TYPE_FILTER_ALL);
            //初始时不设置过滤选项，否则会自动调用SelectedIndexChange函数，导致farpoint重复加载
        ...
        public void FillFarpointCellsWithTunnelBindingRules(int tunnelID, bool select)
        {
            ...
            //遍历Farpoint所有规则
            for (var i = DATA_START_IDX; i < fpRules.Sheets[0].Rows.Count; i++)
            {
        ...
        private bool FillFarpointCellsWithDataset(DataSet ds)
        {
            ...
            for (var i = 0; i < cnt; i++)
            {
                var chkBox = new CheckBoxCellType();
                //选择
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 0].CellType = chkBox;
                //规则编码
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 1].Text =
                    ds.Tables[0].Rows[i][PreWarningRulesDbConstNames.RULE_CODE].ToString();
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 1].Locked = true;
                //规则类别
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 2].Text =
                    ds.Tables[0].Rows[i][PreWarningRulesDbConstNames.RULE_TYPE].ToString();
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 2].Locked = true;
                //预警类型
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 3].Text =
                    ds.Tables[0].Rows[i][PreWarningRulesDbConstNames.WARNING_TYPE].ToString();
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 3].Locked = true;
                //预警级别
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 4].Text =
                    ds.Tables[0].Rows[i][PreWarningRulesDbConstNames.WARNING_LEVEL].ToString();
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 4].Locked = true;
                //适用位置
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 5].Text =
                    ds.Tables[0].Rows[i][PreWarningRulesDbConstNames.SUITABLE_LOCATION].ToString();
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 5].Locked = true;
                //规则描述
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 6].Text =
                    ds.Tables[0].Rows[i][PreWarningRulesDbConstNames.RULE_DESCRIPTION].ToString();
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 6].HorizontalAlignment = CellHorizontalAlignment.Left;
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 6].Locked = true;
                //指标类型
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 7].Text =
                    ds.Tables[0].Rows[i][PreWarningRulesDbConstNames.INDICATOR_TYPE].ToString();
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 7].Locked = true;
                //比较符
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 8].Text =
                    ds.Tables[0].Rows[i][PreWarningRulesDbConstNames.OPERATOR].ToString();
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 8].Locked = true;
                //修改日期
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 9].Text =
                    Convert.ToDateTime(ds.Tables[0].Rows[i][PreWarningRulesDbConstNames.MODIFY_DATE]).ToString();
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 9].Locked = true;
                //备注
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 10].Text =
                    ds.Tables[0].Rows[i][PreWarningRulesDbConstNames.REMARKS].ToString();
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 10].Locked = true;
                //规则ID
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 11].Text =
                    ds.Tables[0].Rows[i][PreWarningRulesDbConstNames.RULE_ID].ToString();
                //绑定的单一规则
                fpRules.Sheets[0].Cells[DATA_START_IDX + i, 12].Text =
                    ds.Tables[0].Rows[i][PreWarningRulesDbConstNames.BINDING_SINGLERULES].ToString();
        ...
        private void PreWarningRulesManagement_Load(object sender, EventArgs e)
        {
            ...
            fpRules.ActiveSheet.Columns[12].Visible = false;
        }
        ...
        private void fpRules_CellClick(object sender, CellClickEventArgs e)
        {
            //判断单击的单元格所在的行是否是有效区域
            ...
            var ent = GetEntityFromFarpointRow(e.Row);
        ...
        /// <param name="vl"></param>
        /// <returns></returns>
        private bool IsPropertyGridValueOK(object vl)
        {
            double numVl = 0;
        ...
        private void propertyGridRules_PropertyValueChanged(object s, PropertyValueChangedEventArgs e)
        {
            if (!IsPropertyGridValueOK(e.ChangedItem.Value))
            {
                ...
                var ent = GetEntityFromFarpointRow(fpRules.Sheets[0].GetSelection(0).Row);
                SetPropertyGridEnt(ent);
                return;
            }
        ...
        public void ClearSelectedRules()
        {
            ...
            for (var i = DATA_START_IDX; i < rows; i++)
            {
                ...
                {
                    var curRuleInFp = fpRules.Sheets[0].Cells[i, 1].Value.ToString();
                    fpRules.Sheets[0].Cells[i, 0].Value = false;
                }
        ...
        {
            var bResult = FileExport.fileExport(fpRules, true);
            return bResult;
        }

        private void cmbFilterLocation_SelectedIndexChanged(object sender, EventArgs e)
        {
            var location = ConvertSuitableLocationFilterStr2Enum(cmbFilterLocation.Text);
            //清空先前单元格中的内容
            ClearFarpointCells();
        ...
        {
            var ret = RULE_TYPE_LOCATION_FILTER.WHOLE;
        ...
        {
            var ret = RULE_TYPE_WARNING_TYPE_FILTER.ALL;
        ...
        {
            var ret = new PreWarningRules(Convert.ToInt32(_ruleCodeId.Value));
            ...
            ret.StrBindingSingleRuleName = _multiRulesBindingSingleRule.Value != null
                ? _multiRulesBindingSingleRule.Value.ToString()
                : ""; //MARK FIELD
            var htNewParams = new Hashtable();
            var n = _paramProp.Count;
            for (var i = 0; i < n; i++)
            {
        ...
        private void farpointFilter1_OnCheckFilterChanged(object sender, EventArgs arg)
        {
            ...
            else //未选中时，根据用户自定义的颜色进行分类显示
            {
                ...
                //设置自定义过滤条件
                FarpointDefaultPropertiesSetter.SetFpCustomFilterProperties(fpRules,
        ...
        {
            //清空过滤条件
            fpRules.ActiveSheet.RowFilter.ResetFilter();
        }

        private void farpointFilter1_OnClickFitColorBtnOK(object sender, EventArgs arg)
        {
            //根据新的颜色值设置自动隐藏过滤条件
            FarpointDefaultPropertiesSetter.SetFpCustomFilterProperties(fpRules, farpointFilter1.GetSelectedFitColor(),
