            ...
            if (iSelCnt > 0)
            {
                ...
                for (int i = 0; i < iSelCnt; i++)
                {
                    ...
                    if (int.TryParse(ds.Tables[0].Rows[i][GasGushQuantityDbConstNames.TUNNEL_ID].ToString(), out iTunnelID))
                    {
                        ...
                        {
                            // 矿井名称
                            this.fpProbeInfo.Sheets[0].Cells[_iRowDetailStartIndex + i, index - 4].Text = tunnelEntity.WorkingFace.MiningArea.Horizontal.Mine.MineName;
                            // 水平
                            this.fpProbeInfo.Sheets[0].Cells[_iRowDetailStartIndex + i, index - 3].Text = tunnelEntity.WorkingFace.MiningArea.Horizontal.HorizontalName;
                            // 采区
                            this.fpProbeInfo.Sheets[0].Cells[_iRowDetailStartIndex + i, index - 2].Text = tunnelEntity.WorkingFace.MiningArea.MiningAreaName;
                            // 工作面
                            this.fpProbeInfo.Sheets[0].Cells[_iRowDetailStartIndex + i, index - 1].Text = tunnelEntity.WorkingFace.WorkingFaceName;
                            // 巷道名称
